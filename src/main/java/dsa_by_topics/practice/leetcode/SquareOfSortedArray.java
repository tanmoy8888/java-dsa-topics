...
public class SquareOfSortedArray {
    ...
    public int[] sortedSquares(int[] nums) {
        ...
        System.out.println("Arrays.toString(positive) = " + Arrays.toString(positive));
        while (m < negative_reverse.length && n < positive.length) {
            if (negative_reverse[m] < positive[n]) {
                nums[k] = negative_reverse[m];
                m++;
            } else {
                nums[k] = positive[n];
                n++;
        ...
        while (m < negative_reverse.length) {
            nums[k] = negative_reverse[m];
            m++;
            k++;
        }
        while (n < positive.length) {
            nums[k] = positive[n];
            n++;
            k++;
        }
